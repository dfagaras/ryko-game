(() => {
  ...
  function configureLaserGridInputs(level){const board=M.boardForLevel(level);[['laserFromColumn',board.columns],['laserToColumn',board.columns],['laserFromRow',board.rows],['laserToRow',board.rows]].forEach(([id,max])=>{const input=$(id);if(input)input.max=String(max)})}
  ...
  $('addLaser').onclick=()=>{
    ...
    if(!Number.isInteger(fromColumn)||!Number.isInteger(fromRow)||!Number.isInteger(toColumn)||!Number.isInteger(toRow)||fromColumn<1||fromColumn>board.columns||toColumn<1||toColumn>board.columns||fromRow<1||fromRow>board.rows||toRow<1||toRow>board.rows||!Number.isFinite(onSeconds)||!Number.isFinite(offSeconds)||!Number.isFinite(startDelay)||onSeconds<=0||offSeconds<=0||startDelay<0)return alert(`Laser cells must be inside C1-C${board.columns}, R1-R${board.rows}, with valid timing.`);
    if(fromColumn===toColumn&&fromRow===toRow)return alert('Laser start and end cells must be different.');
    const from=M.laserPointForCell(l,fromColumn-1,fromRow-1),to=M.laserPointForCell(l,toColumn-1,toRow-1);if(!from||!to)return alert('Could not map laser cells to the board.');
