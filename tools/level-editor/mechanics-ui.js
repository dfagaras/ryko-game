(() => {
  ...
  function configureLaserGridInputs(level){const board=M.boardForLevel(level);[['laserFromColumn',board.columns+1],['laserToColumn',board.columns+1],['laserFromRow',board.rows+1],['laserToRow',board.rows+1]].forEach(([id,max])=>{const input=$(id);if(input)input.max=String(max)})}
  function laserPreviewPoint(level,point){const board=M.boardForLevel(level),logicalX=board.boardLeft+point.x*(board.boardRight-board.boardLeft),logicalY=board.boardTop+point.y*(board.launchLineY-board.boardTop);return{x:(logicalX-board.gridX)/board.gridWidth,y:(logicalY-board.gridY)/board.gridHeight}}
  function render(){const l=read();configureLaserGridInputs(l);const dg=$('dirGrid');dg.innerHTML='';M.MECHANIC_DIRECTIONS.forEach(d=>{const b=document.createElement('button');b.className=`button ghost small mechanic-tool ${armed===`launcher:${d}`?'active':''}`;b.textContent=GL[d];b.onclick=()=>{armed=`launcher:${d}`;sessionStorage.setItem(ARMED_KEY,armed);render()};dg.append(b)}); document.querySelectorAll('.mechanic-mark,.mechanic-shield').forEach(n=>n.remove()); boardGrid.querySelectorAll(':scope>.mechanic-laser-layer').forEach(n=>n.remove());
    ...
    if(l.mechanics.lasers.length){const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.setAttribute('class','mechanic-laser-layer');svg.setAttribute('viewBox','0 0 1000 1000');svg.setAttribute('preserveAspectRatio','none');l.mechanics.lasers.forEach(x=>{const from=laserPreviewPoint(l,x.from),to=laserPreviewPoint(l,x.to),ln=document.createElementNS('http://www.w3.org/2000/svg','line');['x1','y1','x2','y2'].forEach((k,i)=>ln.setAttribute(k,String([from.x,from.y,to.x,to.y][i]*1000)));ln.setAttribute('class','mechanic-laser-line');svg.append(ln)});boardGrid.append(svg)}
    lists(l);
  }
  ...
  $('addLaser').onclick=()=>{
    const l=read(),board=M.boardForLevel(l);
    const fromColumn=+$('laserFromColumn').value,fromRow=+$('laserFromRow').value,toColumn=+$('laserToColumn').value,toRow=+$('laserToRow').value;
    const onSeconds=+$('laserOnSeconds').value,offSeconds=+$('laserOffSeconds').value,startDelay=+$('laserStartDelay').value;
    if(!Number.isInteger(fromColumn)||!Number.isInteger(fromRow)||!Number.isInteger(toColumn)||!Number.isInteger(toRow)||fromColumn<1||fromColumn>board.columns+1||toColumn<1||toColumn>board.columns+1||fromRow<1||fromRow>board.rows+1||toRow<1||toRow>board.rows+1||!Number.isFinite(onSeconds)||!Number.isFinite(offSeconds)||!Number.isFinite(startDelay)||onSeconds<=0||offSeconds<=0||startDelay<0)return alert(`Laser grid lines must be inside C1-C${board.columns+1}, R1-R${board.rows+1}, with valid timing.`);
    if(fromColumn===toColumn&&fromRow===toRow)return alert('Laser start and end grid lines must be different.');
    const from=M.laserPointForGridLine(l,fromColumn-1,fromRow-1),to=M.laserPointForGridLine(l,toColumn-1,toRow-1);if(!from||!to)return alert('Could not map laser grid lines to the board.');
    l.mechanics.lasers.push({id:next(l.mechanics.lasers,'laser'),from,to,onSeconds,offSeconds,startDelay,startsOn:$('laserStartsOn').value==='on'});save(l)
  };
